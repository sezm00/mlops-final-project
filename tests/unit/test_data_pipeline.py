...
def real_pipeline_params():
    ...
    if not os.path.exists(params_path):
        raise FileNotFoundError(f"Critical configuration file missing at: {params_path}")
...
def test_prepare_clean_and_target_encoding(real_pipeline_params):
    ...
    raw_df = pd.DataFrame({
        ...
        "TotalCharges": [" ", "1889.5"],  # Testing numeric string whitespace coercion
        "Churn": ["No", "Yes"]
    })
    ...
    assert cleaned_df.loc[0, "OnlineSecurity"] == "No"  # Verify category consolidation mapping
...
def test_scikit_learn_preprocessing_pipeline(real_pipeline_params):
    ...
    assert not np.isnan(transformed_matrix).any(), "NaN values found after imputation layer!"
